...
describe('MCP Error Scenarios E2E', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'agentsync-errors-'));
  ...
  it('should error when adding non-existent MCP', async () => {
    await expect(addMCP('nonexistent-mcp')).rejects.toThrow(/not found in global registry/);
  });
  ...
  it('should allow removing last MCP (empty config is valid)', async () => {
    ...
    const configAfter = await fs.readJson('agentsync.local.json');
  ...
  it('should handle spaces in paths correctly', async () => {
    ...
    const cursorMcp = await fs.readJson(path.join(spacedDir, '.cursor/mcp.json'));
