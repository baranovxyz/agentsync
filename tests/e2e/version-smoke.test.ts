...
function packPrebuiltArtifact(packDir: string): string {
  ...
  if (result.error) {
    throw new Error(`npm pack failed to start: ${result.error.message}`);
  }
  if (result.status !== 0) {
    throw new Error(
      `npm pack failed (exit ${result.status ?? "unknown"}): ` +
