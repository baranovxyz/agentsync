...
describe('MCP E2E Workflow', () => {
  ...
  it('completes full workflow: add → sync → list → remove', async () => {
    ...
    expect(cursorMcp.mcpServers.github.env.GITHUB_TOKEN).toBe('ghp_e2e_test_token');
    ...
    expect(claudeMcp.postgres.env.POSTGRES_URL).toBe('postgresql://localhost:5432/e2e_db');
