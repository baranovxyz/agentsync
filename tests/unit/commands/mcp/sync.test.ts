...
describe('syncMCP', () => {
  ...
  it('syncs selected MCPs to Cursor target', async () => {
    ...
    expect(cursorMcp.mcpServers.postgres.env.POSTGRES_URL).toBe('postgresql://localhost/db');
  });
  ...
  it('throws error if required environment variable is missing', async () => {
    ...
    await expect(syncMCP()).rejects.toThrow(/Missing environment variable: GITHUB_TOKEN/);
  });
  ...
  it('applies config overrides correctly', async () => {
    ...
    expect(cursorMcp.mcpServers.postgres.env.POSTGRES_URL).toBe('postgresql://localhost/custom_db');
  });
