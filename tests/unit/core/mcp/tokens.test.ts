...
import { substituteTokens, validateTokens, substituteAllMCPs, type MCP } from '../../../../src/core/mcp/tokens.js';
...
describe('substituteTokens', () => {
  ...
  it('throws error for missing environment variable', () => {
    ...
    expect(() => substituteTokens(mcp, env)).toThrow('Missing environment variable: GITHUB_TOKEN');
  });
  ...
  it('replaces tokens in nested string values', () => {
    ...
    expect(result.env?.CONNECTION_STRING).toBe('postgresql://admin:secret123@localhost/mydb');
  });
...
describe('validateTokens', () => {
  ...
  it('throws error listing all missing tokens', () => {
    ...
    expect(() => validateTokens(mcps)).toThrow(/Missing required environment variables/);
