...
describe("SourceResolver", () => {
  ...
  describe("isGitHubSource", () => {
    it("returns true for GitHub sources", () => {
      const githubSources = [
        "github:company/standards",
        "github:company/standards@main",
      ];
      ...
      githubSources.forEach((source) => {
        expect(sourceResolver.isGitHubSource(source)).toBe(true);
      });
    });
    ...
    it("returns false for non-GitHub sources", () => {
      ...
      nonGithubSources.forEach((source) => {
        expect(sourceResolver.isGitHubSource(source)).toBe(false);
      });
    });
  });

  describe("isFilesystemSource", () => {
    it("returns true for filesystem sources", () => {
      const filesystemSources = [
        "/absolute/path",
        "./relative/path",
        "relative/path",
      ...
      filesystemSources.forEach((source) => {
        expect(sourceResolver.isFilesystemSource(source)).toBe(true);
      });
    });

    it("returns false for non-filesystem sources", () => {
      const nonFilesystemSources = [
        "github:company/standards",
        "http://example.com/preset",
        "",
      ];

      nonFilesystemSources.forEach((source) => {
        expect(sourceResolver.isFilesystemSource(source)).toBe(false);
      });
    });
  });
});
