# Anthropic Docs Research Mirror

This directory is a local Markdown mirror of Claude Code documentation discovered from `https://code.claude.com/docs/llms.txt`.

## Refresh Rule

- Before using these files as current documentation, compare today's date with `Last full redownload`.
- If seven or more days have passed, fetch `https://code.claude.com/docs/llms.txt`, compare the index below, and redownload all linked Markdown pages into this directory.
- Do not keep `llms.txt` as a separate local file. Fold the refreshed index into `Available Pages` below so agents have one discovery map.
- Preserve human-added files such as this `AGENTS.md`.
- After a successful refresh, update `Last full redownload` and the `Available Pages` index below.
- Remove the generated `Documentation Index` preheader from downloaded Markdown files if it appears.

## Refresh State

- Last full redownload: 2026-05-03
